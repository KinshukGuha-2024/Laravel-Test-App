...
<head>
    ...
    <title>{{ config('app.name') }} | Add Attachments</title>
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Add Attachments</h1>
                    </div>
                    ...
                    <div class="container-fluid form-contain" style="padding: 20px; height:auto;">
                        <form id="form" class="form" method="POST" enctype="multipart/form-data" action="{{ route('secured.attachment.save.post') }}">
                            @csrf
                            ...
                            <form action="{{ route('secured.attachment.save.post') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div>
                                    ...
                                    <div id="error-message" style="color: red; display: none;">Please select a valid PDF file.</div>
                                </div>
                                <div class="formbold-input-file">
                                    <div class="formbold-filename-wrapper" id="fileList" style="cursor:pointer;">
                                        <!-- Dynamic file names will be inserted here -->
                                    </div>
                                    <label for="upload" class="formbold-input-label" style="cursor:pointer;">
                                        ...
                                        Attach files
                                        <input type="file" name="uploads[]" id="upload" multiple accept=".jpg, .jpeg, .png">
                                    </label>
                                </div>
...
<script>
    function redirectBack() {
        window.location.href="{{ route('secured.attachment.get') }}";
    }
    ...
    document.getElementById('upload').addEventListener('change', function(event) {
        ...
        // Notify the user if no valid files are found
    ...
    function renderFileList() {
        ...
        fileArray.forEach((file, index) => {
            ...
            fileDiv.classList.add('formbold-filename-wrapper');

            const fileDiv2 = document.createElement('div');
            fileDiv2.classList.add('file-div');

            const fileNameSpan = document.createElement('span');
            fileNameSpan.classList.add('formbold-filename');
            ...
            removeButton.addEventListener('click', () => {
                fileArray = fileArray.filter((_, idx) => idx !== index);
                renderFileList();
            });

            fileDiv2.appendChild(fileNameSpan);
            fileDiv2.appendChild(removeButton);
    ...
    function updateFileName() {
        ...
        if (fileInput.files[0].type !== "application/pdf") {
            // If not PDF, display error and reset the input
            document.getElementById("error-message").style.display = "block";
            fileInput.value = "";  // Reset the file input
            document.querySelector('.text span').textContent = 'Click to upload Resume';
            return;
        } else {
            // Hide error message if PDF file is selected
            document.getElementById("error-message").style.display = "none";
            var labelElement = fileInput.closest('label');
            var textElement = labelElement.querySelector('.text span');
            textElement.textContent = fileName; // Update the text to show the file name
        }
    }
